...
public class UsuarioInsertAltera extends Activity implements OnClickListener,
		OnDateSetListener, android.content.DialogInterface.OnClickListener {
	...
	public void salvar() {
		...
		usuario.setPanturrilhaEsquerda(edtPanturrilhaEsquerda.getText()
				.toString());
		usuario.setPanturrilhaDireita(edtPanturrilhaDireita.getText()
				.toString());
	...
	public void onDateSet(DatePicker dialog, int year, int month, int day) {
		if (this.dialogSelecionado == DIALOG_DATA) {
			this.edtData.setText(String.valueOf(day) + "-"
					+ String.valueOf(dialog.getMonth() + 1) + "-"
					+ String.valueOf(year));
		}
	}
	...
	protected Dialog onCreateDialog(int id) {
		...
		if (id == DIALOG_DATA) {
			this.dialogData = new DatePickerDialog(this, this, anoAtual,
					mesAtual, diaAtual);
